    ...
    try {
      ...
      if (targetTeamId) {
        try {
          ...
          // response.teamRanking = teamRanking;
          
          // ユーザーの順位を計算 - メソッドが未実装のためコメントアウト
          // if (teamRanking && teamRanking.ranking) {
          //   const userRank = teamRanking.ranking.findIndex(item => item.userId.toString() === userId);
          //   if (userRank >= 0) {
...
出力形式はJSON形式にしてください。
{
  ...
  "collaborationTips": ["ヒント1", "ヒント2", "ヒント3"]
      ...
      // パース失敗時のフォールバック
      return this.generateTemplateBasedTeamContextAdvice(user, team, teamGoal, dayPillar, fortuneScore);
