...
export class FortuneController {
  ...
  /**
   * チームコンテキスト運勢を取得する
   * @param req リクエスト - パスパラメータとしてチームID(teamId)を受け付ける
   * @param res レスポンス
   */
  public async getTeamContextFortune(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // チームコンテキスト運勢を取得
      const teamContextFortune = targetDate
        ? await fortuneService.getTeamContextFortune(userId, teamId, targetDate)
        : await fortuneService.getTeamContextFortune(userId, teamId);

      res.status(200).json(teamContextFortune);
    } catch (error: any) {
  ...
  public async generateTeamContextFortune(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // リクエストユーザーの権限を確認（チーム管理者または管理者のみ生成可能）
      const requestUser = await User.findById(userId);
      const isTeamAdmin = team.adminId.toString() === userId;
      const isAdmin = requestUser?.role === 'Admin' || requestUser?.role === 'SuperAdmin';

      if (!isTeamAdmin && !isAdmin) {
        res.status(403).json({ error: 'チームコンテキスト運勢の生成権限がありません' });
        return;
      }
      ...
      // 既に運勢データがあり、強制更新フラグがない場合
      if (existingTeamContextFortune && !forceUpdate) {
        res.status(200).json({
          ...existingTeamContextFortune.toObject(),
          message: 'このチームのコンテキスト運勢データは既に生成されています'
        });
        return;
      }

      // チームコンテキスト運勢の生成
      const teamContextFortune = await fortuneService.generateTeamContextFortune(userId, teamId, targetDate, forceUpdate);
      res.status(201).json({
        ...teamContextFortune,
        message: existingTeamContextFortune ? 'チームコンテキスト運勢データを更新しました' : '新しいチームコンテキスト運勢データを生成しました'
      });
    } catch (error: any) {
